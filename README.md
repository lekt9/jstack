...
(verse, applied-lesson) pair to your own **append-only Cloudflare Worker + D1**.
...
- **Read-only** from your side (GET, READ_TOKEN).
...
# 2. Server (one-time): D1 + Worker + secrets + .jesus-loop-env (mode 600)
npm i -g wrangler
wrangler login
./scripts/init-db.sh
```
...
│   ├── init-db.sh           # one-time: D1 + Worker + secrets
│   ├── record-pair.sh       # per-step POST /pairs
│   ├── read-pairs.sh        # read-only CLI
...
├── wrangler.toml
└── .jesus-loop-env    # generated; mode 600; gitignored
```
